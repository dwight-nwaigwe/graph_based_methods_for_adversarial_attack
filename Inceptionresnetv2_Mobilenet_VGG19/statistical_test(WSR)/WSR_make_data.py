...
model_orig=tf.keras.models.load_model('/silenus/PROJECTS/pr-deepneuro/nwaigwed/models//mobilenet.h5',compile=False)
...
def convert_multi_indx_to_int(inp_shape):
    tmparray=list(range(0,inp_shape[0]*inp_shape[1]*inp_shape[2])    )
    tmparray=np.asarray(tmparray)
    tmparray=np.reshape(tmparray,  (inp_shape[0],inp_shape[1],inp_shape[2])    )
    multi_indx_to_int=dict()
    counter=0
    for k in range(0,inp_shape[2]):
        for j in range(0,inp_shape[1]):
            for i in range(0,inp_shape[0]): 
                multi_indx_to_int[(i,j,k) ]=tmparray[i,j,k]
                counter=counter+1     
    return multi_indx_to_int 
...
def get_data(sample_range,chosen_class):
    ...
    for sample_index in sample_range:
        ...
        x_sample_orig= cv2.resize(x_sample_orig, dsize=(224,224), interpolation=cv2.INTER_CUBIC)
        x_sample_orig=np.expand_dims(x_sample_orig, 0)
        x_sample_orig =  tf.keras.applications.mobilenet.preprocess_input(x_sample_orig)
        x_sample_orig=model_star(x_sample_orig)
        ...
        x_sample_adv=model_star(x_sample_adv)
        ...
        if model_correct:
            ...
            if metric=='edges':
                lines2=[]
                adj_matrix=np.zeros([10,1010], 'float32')
