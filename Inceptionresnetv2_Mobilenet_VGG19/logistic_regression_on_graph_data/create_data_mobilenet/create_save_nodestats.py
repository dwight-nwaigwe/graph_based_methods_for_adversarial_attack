...
model_orig=tf.keras.models.load_model('C:\models\inceptionresnetv2.h5',compile=False)
model_orig.layers[-1].activation=tf.nn.softmax

inputs=keras.Input(shape=input_shape)
model_star= keras.Model(inputs=model_orig.input, outputs= model_orig.layers[-4].output)

model = keras.Sequential(
    [
        keras.Input(shape= model_orig.layers[-4].output_shape[1] ),
...
model.layers[0].set_weights( model_orig.layers[-3].get_weights() )
model.layers[1].set_weights( model_orig.layers[-2].get_weights() )
model.layers[2].set_weights( model_orig.layers[-1].get_weights() )

model.layers[0]._name='fc1'
model.layers[1]._name='fc2'
model.layers[2]._name='fc3'   

#This takes as input the the layerwise relevance propogation values and returns a networkx graph created
#from those values. The argument rel_weights is a list of numpy arrays, with each entry of the list being the weights for edges, 
#as calculated by layerwise relevance propagation.
...
def compute_metric( normalize_for_degree=False):
    ...
    for sample_index in range(int(sys.argv[1]),int(sys.argv[2])):
        ...
        x_benign_sample= cv2.resize(x_benign_sample, dsize=(224,224), interpolation=cv2.INTER_CUBIC)
        x_benign_sample=np.expand_dims(x_benign_sample, 0)
        x_benign_sample =   tf.keras.applications.mobilenet.preprocess_input(x_benign_sample)
        x_benign_sample=model_star(x_benign_sample)
        ...
        x_adv_sample=model_star(x_adv_sample)
...
threshold=0.01
outpath='/silenus/PROJECTS/pr-deepneuro/nwaigwed/'+model_name+ '/output'+str(threshold)+'/'
outpath2='/silenus/PROJECTS/pr-deepneuro/nwaigwed/' +model_name+  '/output'+str(threshold)+ '_pure_benign/'
